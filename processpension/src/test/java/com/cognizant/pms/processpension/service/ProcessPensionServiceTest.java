...
public class ProcessPensionServiceTest {
	...
	public void initialize() throws IOException, ParseException {
		when(authClient.validateAuthToken(CommonTestConstants.INVALID_JWT_TOKEN))
				.thenReturn(new ApiResponse(0, null, false));
		when(authClient.validateAuthToken(CommonTestConstants.VALID_JWT_TOKEN))
				.thenReturn(new ApiResponse(0, null, true));
		...
		when(gpdClient.getPensionerDetails(CommonTestConstants.VALID_JWT_TOKEN, "520552812599"))
				.thenReturn(new ApiResponsePd(200, "Data fetched", new PensionerDetails(
						new Pensioner("520552812599", "Subhajit", new Date(), null, 30000D, 5000D, "SELF", null),
	...
	public void testProcessPension() {
		try (MockedStatic<CalculatePensionUtil> mockStatic = Mockito.mockStatic(CalculatePensionUtil.class)) {
			mockStatic.when(() -> CalculatePensionUtil.calculatePension("SELF", "SBI", "PUBLIC", 30000D, 5000D))
					.thenReturn(100D);
			mockStatic.when(() -> CalculatePensionUtil.getBankServiceChargeFromCSV("SBI")).thenReturn(500D);
			when(pensionerRepository.saveAndFlush(ArgumentMatchers.any(Pensioner.class))).thenReturn(new Pensioner());
			when(pensionDetailRepository.saveAndFlush(ArgumentMatchers.any(PensionDetail.class)))
					.thenReturn(new PensionDetail());
